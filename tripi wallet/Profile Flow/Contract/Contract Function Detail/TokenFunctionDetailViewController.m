...
- (void)showNotEnoughFeeAlertWithEstimatedFee:(NSDecimalNumber *) estimatedFee {
	...
	NSString *errorString = [NSString stringWithFormat:@"Insufficient fee. Please use minimum of %@ TRIPI", estimatedFee];
...
- (void)didChangeFeeSlider:(UISlider *) slider {
	...
	TRIPIBigNumber *bigNumber = [TRIPIBigNumber decimalWithString:sliderValue.stringValue];
	self.FEE = bigNumber;
	self.feeView.feeTextField.text = self.FEE.stringValue;
}
...
- (void)normalizeFee {
	...
	TRIPIBigNumber *feeValue = [TRIPIBigNumber decimalWithString:self.feeView.feeTextField.text];
...
- (IBAction)didPressedCallAction:(id) sender {
    ...
    if (self.amountTextField.text.length > 0) {
        amount = [TRIPIBigNumber decimalWithString:self.amountTextField.text];
    } else {
