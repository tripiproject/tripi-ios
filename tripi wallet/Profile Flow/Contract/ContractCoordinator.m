...
-(void)updatePublishedContracts {
    ...
    if (self.publisedContractsOutput) {
        ...
        dispatch_async(dispatch_queue_create("com.example.tripi.contract.update", DISPATCH_QUEUE_CONCURRENT), ^{
    ...
    [SLocator.callContractFacadeService createSmartContractWithTemplate:self.templateModel.path
                                                             ...
                                                             andHandler:^(TransactionManagerErrorType errorType, BTCTransaction *transaction, NSString *hashTransaction, TRIPIBigNumber *estimatedValue) {
                                                                 ...
                                                                 if (errorType == TransactionManagerErrorTypeNone) {
                                                                     ...
                                                                     if (errorType == TransactionManagerErrorTypeNotEnoughFee) {
                                                                         errorString = [NSString stringWithFormat:@"Insufficient fee. Please use minimum of %@ TRIPI", estimatedValue];
                                                                     }
                                                                     if (errorType == TransactionManagerErrorTypeNotEnoughGasLimit) {
                                                                         errorString = [NSString stringWithFormat:@"Insufficient gas limit. Please use minimum of %@ TRIPI", estimatedValue];
                                                                     }
                                                                     switch (errorType) {
                                                                         ...
                                                                         case TransactionManagerErrorTypeInvalidAddress:
                                                                             [weakSelf.createFinishViewController showErrorPopUp:NSLocalizedString(@"Invalid TRIPI Address", nil)];
                                                                             break;
...
- (void)showStatusOfPayment:(TransactionManagerErrorType) errorType {
	...
	switch (errorType) {
		...
		case TransactionManagerErrorTypeNotEnoughMoney:
			...
			break;
		case TransactionManagerErrorTypeInvalidAddress:
			[self.functionDetailController showErrorPopUp:NSLocalizedString(@"Invalid TRIPI Address", nil)];
			break;
		case TransactionManagerErrorTypeNotEnoughMoneyOnAddress:
...
- (void)showStatusOfPayment:(TransactionManagerErrorType) errorType withEstimateFee:(TRIPIBigNumber *) estimatedFee {
