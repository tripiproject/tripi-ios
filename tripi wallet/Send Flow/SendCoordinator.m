...
- (void)start {
	...
	[SLocator.transactionManager getFeePerKbWithHandler:^(TRIPIBigNumber *feePerKb) {

		TRIPIBigNumber *minFee = [feePerKb roundedNumberWithScale:5];
		TRIPIBigNumber *maxFee = SLocator.paymentValuesManager.maxFee;
...
- (void)setForSendSendInfoItem:(SendInfoItem *) item {
	...
	self.fromAddressKey = item.fromTripiAddressKey;
    
    if (item.fromTripiAddress) {
        self.fromAddressString = item.fromTripiAddress;
        ...
        for (ContracBalancesObject* info in tokenBalanceInfo) {
            if ([info.addressString isEqualToString:item.fromTripiAddress]) {
...
- (void)setForToken:(Contract *) aToken withAddress:(NSString *) address andAmount:(NSString *) amount {
	...
	SendInfoItem *item = [[SendInfoItem alloc] initWithTripiAddress:address tokenAddress:aToken.contractAddress amountString:amount];
...
-(NewPaymentOutputEntity*)entityWithSendInfo:(SendInfoItem*) item {
    ...
    entity.receiverAddress = item.tripiAddressKey ? [SLocator.walletManager stringAddressFromBTCKey:item.tripiAddressKey] : item.tripiAddress;
...
- (void)payWithWalletWithAddress:(NSString *) address andAmount:(TRIPIBigNumber *) amount andFee:(TRIPIBigNumber *) fee {
...
- (void)payWithTokenWithAddress:(NSString *) address andAmount:(TRIPIBigNumber *) amount fee:(TRIPIBigNumber *) fee gasPrice:(TRIPIBigNumber *) gasPrice gasLimit:(TRIPIBigNumber *) gasLimit {

	TRIPIBigNumber *amounDivByDecimals = [amount numberWithPowerOf10:self.token.decimals];
...
- (void)showStatusOfPayment:(TransactionManagerErrorType) errorType {
	...
	switch (errorType) {
		...
		case TransactionManagerErrorTypeInvalidAddress:
			[self showErrorPopUp:NSLocalizedString(@"Invalid TRIPI Address", nil)];
			break;
...
- (void)showNotEnoughFeeAlertWithEstimatedFee:(TRIPIBigNumber *) estimatedFee {
	...
	NSString *errorString = [NSString stringWithFormat:@"Insufficient fee. Please use minimum of %@ TRIPI", estimatedFee.stringValue];
	[self showErrorPopUp:NSLocalizedString(errorString, nil)];
}

- (void)showStatusOfPayment:(TransactionManagerErrorType) errorType withEstimateGasLimit:(TRIPIBigNumber *) gasLimit {

	NSString *errorString = [NSString stringWithFormat:@"Insufficient gas limit. Please use minimum of %@ TRIPI", gasLimit.stringValue];
	[self showErrorPopUp:NSLocalizedString(errorString, nil)];
}

- (BOOL)isValidAmount:(TRIPIBigNumber *) amount {
...
- (void)didPresseSendActionWithAddress:(NSString *) address andAmount:(TRIPIBigNumber *) amount fee:(TRIPIBigNumber *) fee gasPrice:(TRIPIBigNumber *) gasPrice gasLimit:(TRIPIBigNumber *) gasLimit {
...
- (void)payActionWithAddress:(NSString *) address andAmount:(TRIPIBigNumber *) amount fee:(TRIPIBigNumber *) fee gasPrice:(TRIPIBigNumber *) gasPrice gasLimit:(TRIPIBigNumber *) gasLimit {
