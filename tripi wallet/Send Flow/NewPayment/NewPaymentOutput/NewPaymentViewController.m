...
- (void)viewDidLoad {
	...
	self.tokenTextField.text = NSLocalizedString(@"TRIPI (Default)", @"");
...
- (void)configGasPrice {
	...
	self.gasPrice = [TRIPIBigNumber decimalWithString:@"40"];
...
- (void)configGasLimit {
	...
	self.gasLimit = [TRIPIBigNumber decimalWithString:@"400000000"];
...
- (void)updateWithEtity:(NewPaymentOutputEntity *) entity {
    ...
    if (entity.amount) {
        ...
        if (entity.amount && ![entity.amount isEqualToString:@""]) {
            TRIPIBigNumber *amount = [TRIPIBigNumber decimalWithString:entity.amount];
    ...
    if (!entity.isTokenChoosen) {
        ...
        self.tokenTextField.text = NSLocalizedString(@"TRIPI (Default)", nil);
...
- (void)updateWithTokenChoosen:(BOOL) isChoosen andTokenName:(NSString*) tokenName {
    ...
    if (self.tokenTextField) {
        self.tokenTextField.text = isChoosen ? tokenName: NSLocalizedString(@"TRIPI (Default)", @"");
        self.needUpdateTokenTexfFields = NO;
    } else {
...
- (void)updateQuickInfoOfWalletWithBalance:(TRIPIBigNumber*) balance andUnconfirmedBalance:(TRIPIBigNumber*) unconfirmedBalance {
    ...
    self.balanceSymbolLabel.text = NSLocalizedString(@"TRIPI", @"");
...
- (void)setMinGasPrice:(TRIPIBigNumber *) min andMax:(TRIPIBigNumber *) max step:(long) step {
	...
	self.gasPrice = [TRIPIBigNumber decimalWithString:min.stringValue];
...
- (BOOL)textField:(UITextField *) textField shouldChangeCharactersInRange:(NSRange) range replacementString:(NSString *) string {
    ...
    if (textField == self.amountTextField) {
        ...
        if (isValid) {
            ...
            TRIPIBigNumber *feeValue = [TRIPIBigNumber decimalWithString:resultString];
...
- (IBAction)makePaymentButtonWasPressed:(id) sender {
    ...
    TRIPIBigNumber *amount = [TRIPIBigNumber decimalWithString:self.amountTextField.text];
