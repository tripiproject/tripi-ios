...
- (void)createQRCode {
	...
	NSString *tripiAddress = self.walletAddress;
	NSString *tokenAddress = self.tokenAddress;
	NSString *amountString = [self correctAmountString];

	[QRCodeManager createQRCodeFromPublicAddress:tripiAddress tokenAddress:tokenAddress andAmount:amountString forSize:self.qrCodeImageView.frame.size withCompletionBlock:^(UIImage *image) {
