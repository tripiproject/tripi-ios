...
- (instancetype)initWithNavigationController:(UINavigationController *) navigationController {
	...
	if (self) {
		...
		_requestQueue = dispatch_queue_create ("com.tripi.requestQueue", DISPATCH_QUEUE_SERIAL);
		[self subcribeEvents];
	}
	return self;
}
...
- (void)didSelectUnsupportedTokenTokenIndexPath:(NSIndexPath *) indexPath withItem:(Contract *) item {
    ...
    content.messageString = NSLocalizedString(@"Sorry, this token exceeds the limit of 128 decimals. So it can not be supported in TRIPI app", @"Unsupported token message");
