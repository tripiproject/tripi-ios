...
- (NSArray <WalletBalancesObject *> *)prepareAddresesDataWithResponse:(NSArray *) response {
	...
	for (int i = 0; i < allAddressesArray.count; i++) {
		...
		object.longBalanceStringBalance = [[TRIPIBigNumber decimalWithInteger:0] stringValue];
		object.shortBalanceStringBalance = [[TRIPIBigNumber decimalWithInteger:0] stringValue];
	...
	for (NSDictionary *item in response) {
		...
		TRIPIBigNumber *bigAmount = [TRIPIBigNumber decimalWithString:decimal.stringValue];
		...
		if (address && amountDouble > 0) {
			...
			TRIPIBigNumber *addressAmountDecimalConteiner = [TRIPIBigNumber decimalWithString:object.longBalanceStringBalance];
			TRIPIBigNumber *newBalance = [addressAmountDecimalConteiner add:bigAmount];
...
- (void)makeTransferFromAddress:(NSString *) from toAddress:(NSString *) to withAmount:(NSString *) amount {
	...
	SendInfoItem *item = [[SendInfoItem alloc] initWithTripiAddressKey:self.addressKeyHashTable[to] tokenAddress:nil amountString:amount fromTripiAddressKey:self.addressKeyHashTable[from]];
