...
NSString *const kTokenKeys = @"tripi_token_tokens_keys";
...
-(NSMutableDictionary <NSString *, TRIPIBigNumber *> *)emptyTokenAddressBalancesDict {
    
    NSArray* addresses = SLocator.walletManager.wallet.addressesInRightOrder;
    NSMutableDictionary <NSString *, TRIPIBigNumber *> * emptyTokensBalances = @{}.mutableCopy;
    
    for (int i = 0; i < addresses.count; i++) {
        TRIPIBigNumber* zeroValue = [TRIPIBigNumber decimalWithInteger:0];
		...
		for (NSDictionary *dict in addressBalance[@"balances"]) {
			...
			if (balance) {
				if ([balance isKindOfClass:[NSNumber class]]) {
					newAddressBalance[addressKey] = [[TRIPIBigNumber alloc] initWithString:[(NSNumber *)balance stringValue]];
				} else if ([balance isKindOfClass:[NSString class]]) {
					newAddressBalance[addressKey] = [[TRIPIBigNumber alloc] initWithString:balance];
				}
			}
		}
