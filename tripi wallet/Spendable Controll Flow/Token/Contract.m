...
- (instancetype)init {
	...
	if (self) {
		...
		_unconfirmedBalance = [[TRIPIBigNumber alloc] initWithString:@"0"];
	}
	return self;
}
...
- (void)setAddressBalanceDictionary:(NSDictionary<NSString *, TRIPIBigNumber *> *) addressBalanceDictionary {
...
- (TRIPIBigNumber *)balance {
	...
	if (!_balance) {
		...
		TRIPIBigNumber *balanceDecimal = [[TRIPIBigNumber alloc] initWithString:@"0"];

		for (TRIPIBigNumber *balanceValue in values) {
...
- (nullable instancetype)initWithCoder:(NSCoder *) aDecoder {
	...
	TRIPIBigNumber *decimals = [aDecoder decodeObjectForKey:kContractDecimals];
	TRIPIBigNumber *totalSupply = [aDecoder decodeObjectForKey:kContractTotalSupply];
	...
	if (self) {
		...
		_adresses = adresses;
		_symbol = symbol;
		_decimals = [decimals isKindOfClass:[NSNumber class]] ? [[TRIPIBigNumber alloc] initWithString:[(NSNumber *)decimals stringValue]] : decimals;
		_totalSupply = [totalSupply isKindOfClass:[NSNumber class]] ? [[TRIPIBigNumber alloc] initWithString:[(NSNumber *)decimals stringValue]] : totalSupply;
		_isActive = isActive;
        _addressBalanceDictionary = addressBalanceDictionary;
	}
