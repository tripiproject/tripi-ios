...
- (BOOL)isValidAmountString:(NSString*)string {
    ...
    NSArray<NSTextCheckingResult *> *matches = [regex matchesInString:string options:0 range:NSMakeRange (0, string.length)];
    BOOL match = matches.count > 0;
    TRIPIBigNumber* result = [[TRIPIBigNumber decimalWithString:string] multiply:[TRIPIBigNumber decimalWithInteger:BTCCoin]];
    BOOL comparing = ![result isGreaterThan:[TRIPIBigNumber maxBigNumberWithPowerOfTwo:64 isUnsigned:NO]];
...
- (BOOL)isValidUIntFromString:(NSString*) string withSize:(NSInteger) size withRegex:(NSRegularExpression*) regex {
    ...
    NSArray<NSTextCheckingResult *> *matches = [regex matchesInString:string options:0 range:NSMakeRange (0, string.length)];
    BOOL match = matches.count > 0;
    BOOL comparing = ![[TRIPIBigNumber decimalWithString:string] isGreaterThan:[TRIPIBigNumber maxBigNumberWithPowerOfTwo:size isUnsigned:YES]];
    return match && comparing;
}
...
- (BOOL)isValidIntFromString:(NSString*) string withSize:(NSInteger) size withRegex:(NSRegularExpression*) regex {
    ...
    NSArray<NSTextCheckingResult *> *matches = [regex matchesInString:string options:0 range:NSMakeRange (0, string.length)];
    BOOL match = matches.count > 0;
    BOOL comparing = ![[TRIPIBigNumber decimalWithString:string] isGreaterThan:[TRIPIBigNumber maxBigNumberWithPowerOfTwo:size isUnsigned:NO]];
    return match && comparing;
}
